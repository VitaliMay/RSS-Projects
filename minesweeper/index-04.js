...
let sizeBlock = 'block'  // переменная для игр с размером поля

if (row === 15) sizeBlock = 'block-mid'
if (row === 25) sizeBlock = 'block-big'

function startInfoBlock() {
  document.querySelector('body').innerHTML = `<header class="info-block"></header>`;
  // document.querySelector('.info-block').innerHTML = `<button class="btn-restart">New Game</button>`;
  document.querySelector('.info-block').innerHTML = `<div class="btn-restart">New Game</div>`;
  ...
  document.querySelector('.info-block').innerHTML += `<div class="div-select"></div>`; // добавление элемента для выбора количества мин
  //document.querySelector('.div-select').innerHTML = `<select class="select"></select>`; // добавление элемента для выбора количества мин
  document.querySelector('.div-select').innerHTML = `<div class="select-text">Число мин</div>`; // добавление элемента для выбора количества мин
  document.querySelector('.div-select').innerHTML += `<select class="select"></select>`; // добавление элемента для выбора количества мин
  let bombInfo = ''
  ...
  document.querySelector('.info-block').innerHTML += `<div class="div-flag"><div class="flag-text">Число флагов</div><div class="flagNumber">${flagNumber.length}</div></div>`;

  document.querySelector('.info-block').innerHTML += `<form class="form" action="#" method="get"></form>`;
  /*
  document.querySelector('form').innerHTML = `<div class="form-wrapper">
  <div class="form-text">Размер игрового поля</div>
  <div class="form-input-wrapper">
  <div class="form-input">
  <input checked type="radio" name="board" value="10" />
  10 x 10
  </div>
  <div class="form-input">
  <input type="radio" name="board" value="15" />
  15 x 15
  </div>
  <div class="form-input">
  <input type="radio" name="board" value="25" />
  25 x 25
  </div>
  </div>
  </div>`
  */
  ...
  for (let i = 0; i < 3; i++){
    ...
    formInput += `<div class="form-input"><input type="radio" name="board" value="${temp}" ${checked} />${temp} x ${temp}</div>`
  }
//console.log(formInput)

  document.querySelector('form').innerHTML = `<div class="form-wrapper">
  <div class="form-text">Размер игрового поля</div>
  <div class="form-input-wrapper">${formInput}</div></div>`
...
const selected = document.querySelector('input[name="board"]:checked').value;
...
function start(){
  ...
  const selected = document.querySelector('input[name="board"]:checked').value; // выбор размера поля
  row = Number(selected)
  column = Number(selected)
  //console.log(`input = ${Number(selected)}`)
  if (row === 10) sizeBlock = 'block'
  if (row === 15) sizeBlock = 'block-mid'
  if (row === 25) sizeBlock = 'block-big'
  ...
  matrixDoc = document.querySelectorAll(`.${sizeBlock}`);
  ...
  // Обнуляю количество флагов
  flagNumber = []
  document.querySelector('.flagNumber').innerHTML = `${flagNumber.length}`;
}
...
const right = function(){ // 
  document.querySelector('.playboard').addEventListener('contextmenu', function(event) {
    ...
    if(event.target.matches(`.${sizeBlock}`) && matrix[coorXY.x][coorXY.y].mineOpen !== true) {
      event.target.classList.toggle('guess')
    }
    // можно удалять флаги поставленные в открытые ячейки
    if(event.target.matches(`.${sizeBlock}`) && matrix[coorXY.x][coorXY.y].mineOpen === true) {
      event.target.classList.remove('guess')
    }
    //elem.classList.add/remove("class") 

    // подсчитываю количество расставленных флагов
    flagNumber = document.querySelectorAll(`.guess`);
    console.log(flagNumber.length)
    //document.querySelector('.div-flag').innerHTML = `Число флагов ${flagNumber.length}`;
    document.querySelector('.flagNumber').innerHTML = `${flagNumber.length}`;
...
document.querySelector('.playboard').addEventListener('click', function(event) {
  ...
  if(event.target.matches(`.${sizeBlock}`) && !(event.target.matches('.guess'))) {
    open(event)
  }
})
