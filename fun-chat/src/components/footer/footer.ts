...
const createFooter = (parent: HTMLElement) => {
  ...
  const footerContainer = createEl({ classes: ['footer-container'], parent: footer });
  const footerLinkGithub = createEl({
    tag: 'a',
    classes: ['footer-link', 'footer-container__github'],
    attributes: { target: '_blank', href: 'https://github.com/VitaliMay' },
  ...
  const footerLinkRSS = createEl({
    ...
    attributes: { target: '_blank', href: 'https://rs.school/courses/javascript-ru' },
    parent: footerContainer,
  });
